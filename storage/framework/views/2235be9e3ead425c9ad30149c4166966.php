<?php $__env->startSection('title', 'لوحة الإدارة - أكاديمية مكتب فني'); ?>
<?php $__env->startSection('page_title', 'لوحة الإدارة الأساسية'); ?>

<?php $__env->startSection('sidebar'); ?>
    <a href="<?php echo e(route('admin.users.index')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl text-text-secondary hover:bg-glass-bg hover:text-[#4a662e] transition font-semibold">
...
<?php $__env->startSection('content'); ?>
    ...
    <?php if(session('success')): ?>
        <div id="laravel-toast" class="fixed bottom-8 left-1/2 -translate-x-1/2 bg-gold-light text-black px-6 py-3 rounded-full font-bold shadow-[0_5px_20px_rgba(201,150,58,0.4)] z-50 transition-opacity duration-500"><?php echo e(session('success')); ?></div>
    <?php endif; ?>
    ...
    <div class="space-y-8" id="dashboard-view">
        ...
        <div class="bg-bg-card border border-glass-border rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <div id="courses-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php $__empty_1 = true; $__currentLoopData = $courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="group bg-black/30 border border-glass-border rounded-2xl overflow-hidden hover:border-gold hover:-translate-y-1 transition-all duration-300 course-card" data-course-id="<?php echo e($course->id); ?>" data-course-details="<?php echo e($course->toJson()); ?>">
                        <img src="<?php echo e(asset($course->image_path ? 'storage/' . $course->image_path : 'images/default-course.jpg')); ?>" alt="<?php echo e($course->title); ?>" class="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-500">
                        <div class="p-5">
                            <h3 class="text-lg font-bold text-[#4a662e] mb-2"><?php echo e($course->title); ?></h3>
                            <p class="text-sm text-text-secondary mb-4 leading-relaxed line-clamp-2"><?php echo e($course->description); ?></p>
        ...
        <div class="bg-bg-card border border-glass-border rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <form action="<?php echo e(url('/admin/settings')); ?>" method="POST" class="space-y-6">
                <?php echo csrf_field(); ?>
                ...
                <div class="space-y-2">
                    ...
                    <input type="url" id="master_drive_link" name="master_drive_link" class="w-full px-4 py-3 bg-black/40 border border-glass-border rounded-xl text-text-primary focus:border-gold focus:ring-1 focus:ring-gold transition-colors" value="<?php echo e(old('master_drive_link', $setting->master_drive_link)); ?>" placeholder="https://...">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="url" name="sketchup_link" class="w-full px-4 py-3 bg-black/40 border border-glass-border rounded-xl text-text-primary focus:border-gold focus:ring-1 focus:ring-gold transition-colors" value="<?php echo e(old('sketchup_link', $setting->sketchup_link)); ?>" required>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-text-secondary">رابط 3Ds Max & Corona</label>
                        <input type="url" name="max_link" class="w-full px-4 py-3 bg-black/40 border border-glass-border rounded-xl text-text-primary focus:border-gold focus:ring-1 focus:ring-gold transition-colors" value="<?php echo e(old('max_link', $setting->max_link)); ?>" required>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-text-secondary">رابط Autocad</label>
                        <input type="url" name="autocad_link" class="w-full px-4 py-3 bg-black/40 border border-glass-border rounded-xl text-text-primary focus:border-gold focus:ring-1 focus:ring-gold transition-colors" value="<?php echo e(old('autocad_link', $setting->autocad_link)); ?>" required>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-text-secondary">رابط Manual Sketch</label>
                        <input type="url" name="manual_link" class="w-full px-4 py-3 bg-black/40 border border-glass-border rounded-xl text-text-primary focus:border-gold focus:ring-1 focus:ring-gold transition-colors" value="<?php echo e(old('manual_link', $setting->manual_link)); ?>" required>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-text-secondary">رابط Real Time Landscape</label>
                        <input type="url" name="landscape_link" class="w-full px-4 py-3 bg-black/40 border border-glass-border rounded-xl text-text-primary focus:border-gold focus:ring-1 focus:ring-gold transition-colors" value="<?php echo e(old('landscape_link', $setting->landscape_link)); ?>" required>
                    </div>
                </div>
        ...
        <div class="bg-bg-card border border-glass-border rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <form action="<?php echo e(url('/admin/projects')); ?>" method="POST" enctype="multipart/form-data" class="space-y-4 max-w-2xl">
                <?php echo csrf_field(); ?>
                <div class="space-y-2">
                    ...
                    <tbody class="divide-y divide-glass-border">
                        <?php $__empty_1 = true; $__currentLoopData = $projects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-glass-bg transition-colors">
                                <td class="px-6 py-4">
                                    <img src="<?php echo e(asset('storage/' . $project->image_path)); ?>" alt="Project" class="w-24 h-16 object-cover rounded-lg border border-glass-border">
                                </td>
                                <td class="px-6 py-4">
                                    <a href="<?php echo e($project->facebook_link); ?>" target="_blank" class="text-[#4a662e] hover:underline font-semibold flex items-center gap-1">رابط المنشور <i class="fa-solid fa-external-link text-[10px]"></i></a>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="<?php echo e($project->drive_link); ?>" target="_blank" class="text-[#4a662e] hover:underline font-semibold flex items-center gap-1">رابط الدرايف <i class="fa-solid fa-external-link text-[10px]"></i></a>
                                </td>
                                <td class="px-6 py-4">
                                    <form action="<?php echo e(route('admin.projects.destroy', $project->id)); ?>" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا المشروع؟');">
                                        <?php echo csrf_field(); ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('add-course-btn').addEventListener('click', () => {
            courseForm.reset();
            courseForm.action = '<?php echo e(route('admin.courses.store')); ?>';
        ...
        function createCourseCard(course) {
            const defaultImage = '<?php echo e(asset('images/default-course.jpg')); ?>';
        ...
        function saveOrder() {
            ...
            fetch('<?php echo e(route('admin.sessions.reorder')); ?>', {
                method: 'POST',
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /mnt/Felo/Courses/learing/PHP/Laravel/Gorge/resources/views/Admin/admin.blade.php ENDPATH**/ ?>