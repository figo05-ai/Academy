<?php $__env->startSection('title', 'إدارة المستخدمين - أكاديمية مكتب فني'); ?>
<?php $__env->startSection('page_title', 'إدارة المستخدمين'); ?>

<?php $__env->startSection('sidebar'); ?>
    <a href="<?php echo e(url('/admin')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl text-text-secondary hover:bg-glass-bg hover:text-[#4a662e] transition font-semibold">
...
<div class="space-y-8">
    <div class="bg-bg-card border border-glass-border rounded-2xl p-6 md:p-8 shadow-lg">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            ...
            <form action="<?php echo e(route('admin.users.index')); ?>" method="GET" class="w-full md:w-auto flex gap-2">
                <input type="text" name="search" class="w-full md:w-80 px-4 py-2.5 bg-black/40 border border-glass-border rounded-xl text-text-primary focus:border-gold focus:ring-1 focus:ring-gold transition-colors" placeholder="ابحث بالاسم أو البريد..." value="<?php echo e(request('search')); ?>">
                        ...
                        <tr class="hover:bg-glass-bg transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-bg-secondary flex items-center justify-center font-bold text-[#4a662e] shrink-0"><?php echo e(mb_substr($user->name, 0, 1)); ?></div>
                                    <span class="font-semibold text-text-primary"><?php echo e($user->name); ?></span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-text-primary"><?php echo e($user->email); ?></td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 bg-blue-500/10 text-blue-700 rounded-lg font-bold text-sm"><?php echo e($user->courses->count()); ?> كورسات</span>
                            </td>
                            <td class="px-6 py-4 text-text-primary"><?php echo e($user->created_at->format('Y-m-d')); ?></td>
                            <td class="px-6 py-4">
                                <button class="btn-toggle-status flex items-center gap-2 px-3 py-1.5 rounded-lg font-bold text-sm transition-colors border <?php echo e($user->is_active ? 'bg-green-500/10 text-green-700 border-green-500/30 hover:bg-green-500 hover:text-white' : 'bg-gray-500/10 text-text-secondary border-gray-500/30 hover:bg-text-secondary hover:text-text-primary'); ?>" data-user-id="<?php echo e($user->id); ?>">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    ...
                                    <button class="btn-delete-user px-3 py-1.5 bg-red-500/10 text-red-700 hover:bg-red-500 hover:text-white border border-red-500/30 rounded-lg font-semibold text-sm transition-colors flex items-center gap-2" data-user-id="<?php echo e($user->id); ?>">
                                    ...
                                    <button class="btn-courses px-3 py-1.5 bg-purple-500/10 text-purple-700 hover:bg-purple-500 hover:text-white border border-purple-500/30 rounded-lg font-semibold text-sm transition-colors flex items-center gap-2"
                                            data-user-id="<?php echo e($user->id); ?>"
                                            data-user-name="<?php echo e($user->name); ?>"
                                            data-user-courses="<?php echo e(json_encode($user->courses->pluck('id'))); ?>">
...
document.addEventListener('DOMContentLoaded', function () {
    const allCourses = <?php echo json_encode($allCourses, 15, 512) ?>;
        ...
        if (e.target.closest('.btn-delete-user')) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`/admin/users/${button.dataset.userId}`, { method: 'DELETE', headers: {'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>', 'Accept': 'application/json'} })
    ...
    coursesForm.addEventListener('submit', function(e) {
        ...
        fetch(this.action, { method: 'POST', body: formData, headers: {'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'} })
            .then(res => res.json())
    ...
    editUserForm.addEventListener('submit', function(e) {
        e.preventDefault();
        fetch(this.action, { method: 'POST', body: new FormData(this), headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>', 'Accept': 'application/json' } })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                showToast(data.success);
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /mnt/Felo/Courses/learing/PHP/Laravel/Gorge/resources/views/Admin/users/index.blade.php ENDPATH**/ ?>