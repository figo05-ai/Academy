...
@section('page_title', 'استكشف الكورسات')
    ...
    @forelse ($data as $course)
        <div class="bg-bg-card border border-glass-border rounded-2xl overflow-hidden flex flex-col group hover:-translate-y-2 hover:shadow-[0_12px_35px_rgba(0,0,0,0.4)] hover:border-gold-light/40 transition-all duration-400 cursor-pointer">
            <div class="h-52 overflow-hidden border-b border-glass-border shrink-0">
                <img src="{{ $course->image_path ? asset('storage/' . $course->image_path) : asset('images/default-course.jpg') }}" alt="{{ $course->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
            </div>
            <div class="p-6 flex flex-col flex-grow">
                <h3 class="text-xl font-bold text-text-primary mb-3">{{ $course->title }}</h3>
                <p class="text-sm text-text-secondary leading-relaxed mb-6 flex-grow">{{ Str::limit($course->description, 120) }}</p>
                <div class="pt-4 border-t border-glass-border border-dashed flex items-center justify-between mt-auto">
                    <div class="text-2xl font-black text-[#4a662e]">{{ $course->price }} <span class="text-sm font-normal text-text-secondary">EGP</span></div>
                    <button @click="activeCourse = {{ json_encode($course) }}; modalOpen = true" class="px-5 py-2.5 bg-gradient-to-br from-gold-light to-gold text-text-primary font-bold rounded-xl hover:scale-105 transition-transform shadow-md hover:shadow-gold/30 text-sm">عرض التفاصيل</button>
                </div>
            </div>
